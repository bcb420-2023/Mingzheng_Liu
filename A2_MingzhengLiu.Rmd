...
# note that we were using the organism = hsapiens insted of 
...
knitr::kable(head(enriched_upreg, 10), format = "html")
knitr::kable(head(enriched_downreg, 10), format = "html")

# inspect top one from three data sources for upregulated genes
knitr::kable(rbind(enriched_upreg[enriched_upreg$source == "GO:BP",][1,],
                   enriched_upreg[enriched_upreg$source == "REAC",][1,],
                   enriched_upreg[enriched_upreg$source == "WP",][1,]),
             format = "html")

# inspect top one from three data sources for downregulated genes
knitr::kable(rbind(enriched_downreg[enriched_downreg$source == "GO:BP",][1,],
                   enriched_downreg[enriched_downreg$source == "REAC",][1,],
                   enriched_downreg[enriched_downreg$source == "WP",][1,]),
             format = "html")
```
## Visual Representation of the GSEA results 
```{r enriched_upreg, out.width = "90%", echo = FALSE, fig.cap="Figure 5. Manhattan plot displaying the distribution of the top terms from each data source for upregulated genes of 4 Smo(flox/-) mutants (MUT) and 4 wild type (WT) samples. A extensive number of GP:BP terms compared to REAC and WP, and terms of RNA processing, biological_process, Metabolism, REACTOME root term, and WIKIPATHWAYS are exceeding 16 -log10 correction threshold."}
...
```{r enriched_downreg, out.width = "90%", echo = FALSE, fig.cap="Figure 6. Manhattan plot displaying the distribution of the top terms from each data source for downregulated genes of 4 Smo(flox/-) mutants (MUT) and 4 wild type (WT) samples. A extensive number of REAC terms compared to GP:BP and WP, and terms of biological_process, REACTOME root term, and WIKIPATHWAYS are exceeding 16 -log10 correction threshold."}
# Generate a Manhattan plot to visualize the distribution of the top terms from each data source.
